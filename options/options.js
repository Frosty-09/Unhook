...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadAllSettings() {
    storage.get(DEFAULT_SETTINGS, (settings) => {
      ...
      const optSegmentBtns = document.querySelectorAll(
        "#optPlaylistViewModeSelector .segment-btn",
      );
      optSegmentBtns.forEach((btn) => {
  ...
  // Playlist View Mode Segment Buttons in Options
  const optSegmentBtns = document.querySelectorAll(
    "#optPlaylistViewModeSelector .segment-btn",
  );
  optSegmentBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      optSegmentBtns.forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      const viewMode = btn.dataset.view;
      storage.set({ playlist_view_mode: viewMode });

      // Automatically check full width playlist if user chooses a view
  ...
  if (resetDefaultsBtn) {
    resetDefaultsBtn.addEventListener("click", () => {
      if (
        confirm(
          "Are you sure you want to reset all Unhook settings to factory defaults?",
        )
      ) {
